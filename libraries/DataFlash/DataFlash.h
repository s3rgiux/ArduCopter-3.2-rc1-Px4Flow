...
public:
	...
	////////Agregadas
	void Log_Write_OF(float spx,float vx,float spy,float vy);
	void Log_Write_Alt_Ctrl(float sp_a,float alt_z,float sp_zv,float vz,float spa,float az);
	void Log_Write_XY(float sp_px,float px,float sp_py,float py,float sp_vx,float vx,float sp_vy,float vy);
	void Log_Write_ANG(float sp_ang,float ang,float rate,float ctrl);
...
////Agregadas
struct PACKED log_OF {
    LOG_PACKET_HEADER;
    uint32_t time_ms;
    float sp_x, of_vx, sp_y,of_vy;
};

struct PACKED log_Alt {
    LOG_PACKET_HEADER;
    uint32_t time_ms;
    float sp_alt,alt,sp_vz,vel_z,sp_acc,acc_z;
};

struct PACKED log_XY{
	LOG_PACKET_HEADER;
	uint32_t time_ms;
	float s_px,p_x,s_py,p_y,s_vx,v_x,s_vy,v_y;
};

struct PACKED log_ANG{
	LOG_PACKET_HEADER;
	uint32_t time_ms;
	float sp_ang_d,ang_d,rate_d,ctrl_d;
};
...
#define LOG_COMMON_STRUCTURES \
	...
	{ LOG_OF_MSG, sizeof(log_OF), \
      "OF",  "Iffff",     "TimeMS,SPX,OFX,SPY,OFY" }, \
	{ LOG_ALT_MSG, sizeof(log_Alt), \
      "ALT",  "Iffffff",     "TimeMS,SPAL,ALT,SPVZ,VZ,SPAZ,ACCZ" }, \
	{ LOG_XY_MSG, sizeof(log_XY), \
      "NXY",  "Iffffffff",     "TimeMS,SPX,PX,SPY,PY,SPVX,VX,SPVY,VY" }, \
	{ LOG_ANG_MSG, sizeof(log_ANG), \
      "ANG",  "Iffff",     "TimeMS,SPA,ANG,RTE,CTRL"}, \
...
#define LOG_OF_MSG		  148
#define LOG_ALT_MSG		  149
#define LOG_XY_MSG		  150
#define LOG_ANG_MSG		  151
