...
// Agregadas
void DataFlash_Class::Log_Write_OF(float spx,float vx,float spy,float vy)
{
    uint32_t tstamp = hal.scheduler->millis();
    struct log_OF pkt = {
        LOG_PACKET_HEADER_INIT(LOG_OF_MSG),
        time_ms : tstamp,
        sp_x  : spx,
        of_vx  : vx,
        sp_y  : spy,
...
void DataFlash_Class::Log_Write_XY(float sp_px,float px,float sp_py,float py,float sp_vx,float vx,float sp_vy,float vy)
{
    uint32_t tstamp = hal.scheduler->millis();
    struct log_XY pkt = {
        LOG_PACKET_HEADER_INIT(LOG_XY_MSG),
        time_ms : tstamp,
        s_px  : sp_px,
        p_x  : px,
        s_py  : sp_py,
...
void DataFlash_Class::Log_Write_Alt_Ctrl(float sp_a,float alt_z,float sp_zv,float vz,float spa,float az)
{
    uint32_t tstamp = hal.scheduler->millis();
    struct log_Alt pkt = {
        LOG_PACKET_HEADER_INIT(LOG_ALT_MSG),
        time_ms : tstamp,
        sp_alt  : sp_a,
        alt  : alt_z,
        sp_vz  : sp_zv,
...
void DataFlash_Class::Log_Write_ANG(float sp_ang,float ang,float rate,float ctrl)
{
    uint32_t tstamp = hal.scheduler->millis();
    struct log_ANG pkt = {
